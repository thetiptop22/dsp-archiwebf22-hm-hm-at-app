...
test('GET /api/admin/admin@thetiptop.fr returns object with role=admin', async () => {
  const response = await fetch(`${process.env.HOST}:${process.env.PORT}/api/admin/admin@thetiptop.fr`);
...
test('GET /api/admin/employe@thetiptop.fr returns object with role=employe', async () => {
  const response = await fetch(`${process.env.HOST}:${process.env.PORT}/api/admin/employe@thetiptop.fr`);
  const json = await response.json();
  expect(response.status).toBe(201);
